...
def HSE(hand, board):
    ...
    deck_1 = [card for card in deck if card not in hand]
    deck_2 = [card for card in deck if card not in board]

    # for each possible hand of the opponent
    for i in range(len(deck_2)):
        for j in range(i+i, len(deck_2)):
            villain_hand = [deck_2[i], deck_2[j]]

            hero_all_card = hand + board
            villain_all_card = villain_hand + board

            hero_best_hand, hero_best_rank = get_best_hand(hero_all_card)
            villain_best_hand, villain_best_rank = get_best_hand(villain_all_card)

            if hero_best_rank > villain_best_rank:
                wins += 1
            elif hero_best_rank < villain_best_rank:
                losses += 1
            else:
                ties += 1
